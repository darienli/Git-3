...
tree* find(tree* tr, int x) {//поиск
    if (!tr || x == tr->inf)//нашли или дошли до конца ветки
        return tr;
    if (x < tr->inf) //если хотим найти элемент, который меньше текущего, тогда идем по левой ветке 
...
void Delete(tree*& tr, tree* v) {//удаление узла
    ...
    if (!p && !v->left && !v->right) tr = NULL; //дерево содержит один узел
    else if (!v->left && !v->right) {//если нет детей, но есть родитель
        ...
        if (!p) { //если удаляем КОРЕНЬ, у которого 1 ребенок (то есть v - корень поддерева) (зачем это проверять, если мы в строке 106 убедились, что это не так;
